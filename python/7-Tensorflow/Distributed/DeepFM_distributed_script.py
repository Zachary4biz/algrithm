...
def main(unused_argv):
    ...
    if FLAGS.job_name == 'ps':
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index,cluster=cluster)):
            ...
            path = "/data/houcunyue/zhoutong/data/CriteoData/test_data/train_splitted_0{task_index}.txt".format(task_index=FLAGS.task_index)
            print_t("加载的文件路径是: %s" % path)
            data_manager = DataManager(path=path.format(task_index=FLAGS.task_index),task_index=FLAGS.task_index,label_col='target')
...
class DataManager(object):
    def __init__(self, path, task_index, label_col ='target'):
        self.label_col=label_col
        self.task_index = task_index
        # 分块读取文件,然后concat, 划分训练集、测试集
        print_t("   loading data ... ")
        train_data,test_data,feature_dict = self.prepare(path)
        self.Xi_train, self.Xv_train, self.y_train = self.parse(train_data,fd=feature_dict)
        self.Xi_valid, self.Xv_valid, self.y_valid = self.parse(test_data,fd=feature_dict)
        # 需要做类型转换
        self.ignor_cols = ['']
        self.feature_size = feature_dict.feat_dim
