...
def print_t(param):
        sys.stdout.flush()
        now = time_parent.strftime("|%Y-%m-%d %H:%M:%S| ", time_parent.localtime(time_parent.time()))
...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        if self.optimizer_type == "adam":
            ...
            self.opt = YFOptimizer(learning_rate=self.learning_rate, momentum=0.0)  # .minimize(self.loss)
    ...
    def fit(self, Xi_train, Xv_train, y_train,
        ...
        print_t("epoch is %s" % str(self.epoch))
        for epoch in range(self.epoch):
            print_t("current_epoch is %s" % str(epoch))
            t1 = time()
            ...
            for i in range(total_batch):
                ...
                sys.stdout.flush()
                sys.stdout.write(now+": "+"batch: %s/%s   " % (i,total_batch))
                sys.stdout.flush()
                Xi_batch, Xv_batch, y_batch = self.get_batch(Xi_train, Xv_train, y_train, self.batch_size, i)
                self.fit_on_batch(Xi_batch, Xv_batch, y_batch)
            print_t("fit_on_batch done.")
            ...
            if self.verbose > 0 and epoch % self.verbose == 0:
                if has_valid:
                    print_t("[%d] train-result=%.4f, valid-result=%.4f [%.1f s]"
                          % (epoch + 1, train_result, valid_result, time() - t1))
                else:
                    print_t("[%d] train-result=%.4f [%.1f s]"
